...
const sectionProjection = { _id: 1, coords: 1, distance: 1, rg_fv_graph_dbl: 1, rg_fv_graph_nd: 1, rg_fv_graph_na: 1 };
export type Section = Pick<dbSections, keyof typeof sectionProjection>;
...
async function run() {
  ...
  async function queryData() {
    ...
    const sections = (await sectionsModel
      .find<HydratedDocument<DocumentType<Section>>>({}, sectionProjection)
      .lean()
    ...
    const stops = (
      (await stopsModel
        .find<HydratedDocument<DocumentType<Stop>>>({ coords: { $not: { $elemMatch: { $eq: Infinity } } } }, stopProjection)
        .lean()
        // Coords field type lost...
  ...
  function makeGraph() {
    ...
    for (const s of sections) {
      ...
      s.rg_fv_graph_dbl
        ...
        : footGraph.add_arc(s.rg_fv_graph_nd as number, s.rg_fv_graph_na as number, s.distance);
    }
  ...
  function updateGraph() {
    ...
    for (let i = 0; i < approachedStops.length; i++) {
      ...
      footGraph.remove_arc(section.rg_fv_graph_nd as number, section.rg_fv_graph_na as number);

      //Insert new node approachedStop
      //Create arc from p1 to approachedStop to p2
      //Ensure unique node id for approachedStop
      footGraph.add_arc(section.rg_fv_graph_nd as number, `stop-${stops[i]._id}`, toApproadchedStop);
      footGraph.add_arc(`stop-${stops[i]._id}`, section.rg_fv_graph_na as number, fromApproachedStop);
    }
  }
  ...
  function computePaths() {
    return new Promise((res, rej) => {
      ...
      for (let i = 0; i < stops.length; i++) {
        workerPool
          .run<typeof computePath<false>>([`stop-${stops[i]._id}`, false])
          .then((sourcePaths) => {
