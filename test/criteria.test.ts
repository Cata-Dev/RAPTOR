...
describe("Foot distance", () => {
  const footDistanceTyped: Criterion<number, number, number, "footDistance"> = footDistance;

  test("Naming", () => {
    expect(footDistanceTyped.name).toBe("footDistance");
  });

  const originJS: JourneyStep<number, number, number, [[number, "footDistance"]], "DEPARTURE"> = makeJSComparable({
    label: new Label<number, number, number, [[number, "footDistance"]]>([footDistanceTyped], 0),
  });

  const vehicleJourneyStep = makeJSComparable<number, number, number, [[number, "footDistance"]], "VEHICLE">({
  ...
  const footJourneyStep1 = makeJSComparable<number, number, number, [[number, "footDistance"]], "FOOT">({
    boardedAt: [0, originJS],
    transfer: { to: 1, length: 3 },
    label: setLabelValues(new Label<number, number, number, [[number, "footDistance"]]>([footDistanceTyped], 0), [4]),
  });

  const footJourneyStep2 = makeJSComparable<number, number, number, [[number, "footDistance"]], "FOOT">({
    boardedAt: [0, footJourneyStep1],
    transfer: { to: 0, length: 5 },
    label: setLabelValues(new Label<number, number, number, [[number, "footDistance"]]>([footDistanceTyped], 0), [6]),
  });
...
describe("Buffer time", () => {
  const bufferTimeTyped: Criterion<number, number, number, "bufferTime"> = bufferTime;

  test("Naming", () => {
    expect(bufferTimeTyped.name).toBe("bufferTime");
  });

  const originJS: JourneyStep<number, number, number, [[number, "bufferTime"]], "DEPARTURE"> = makeJSComparable({
    label: new Label<number, number, number, [[number, "bufferTime"]]>([bufferTimeTyped], 0),
  });

  const footJourneyStep = makeJSComparable<number, number, number, [[number, "bufferTime"]], "FOOT">({
    boardedAt: [0, originJS],
    transfer: { to: 0, length: 3 },
    label: new Label<number, number, number, [[number, "bufferTime"]]>([bufferTimeTyped], 0),
  });

  const vehicleJourneyStep1 = makeJSComparable<number, number, number, [[number, "bufferTime"]], "VEHICLE">({
    boardedAt: [0, originJS],
    route: new Route(0, [0], [{ id: 0, times: [[4, 5]] }]),
    tripIndex: 0,
    label: setLabelValues(new Label<number, number, number, [[number, "bufferTime"]]>([bufferTimeTyped], 4), [-5]),
  });

  const vehicleJourneyStep2 = makeJSComparable<number, number, number, [[number, "bufferTime"]], "VEHICLE">({
    boardedAt: [0, vehicleJourneyStep1],
    route: new Route(1, [0], [{ id: 0, times: [[7, 8]] }]),
    tripIndex: 0,
    label: setLabelValues(new Label<number, number, number, [[number, "bufferTime"]]>([bufferTimeTyped], 8), [-4]),
  });
